...
class Build {
    ...
    static async find(conditions = {}, sort = { createdAt: -1 }, limit) {
        try {
    ...
    static async create(buildData) {
        try {
            ...
            const result = await this.pool.query(`INSERT INTO builds 
         (provider, build_time, start_time, end_time, success, logs, metadata) 
         VALUES ($1, $2, $3, $4, $5, $6, $7) 
         RETURNING *`, [provider, buildTime, startTime, endTime, success, logs || null, metadata ? JSON.stringify(metadata) : null]);
            return this.mapRowToBuild(result.rows[0]);
        }
        catch (error) {
